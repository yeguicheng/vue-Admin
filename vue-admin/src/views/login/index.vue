<template>
	<div id="login">
		<div class="middle">
			<ul class="top">
				<li @click="Active(item)" :class="['loginName',item.CurrentFlag === isShow?'active':null]" v-for="(item,ind) of list"
				 :key="ind">{{item.title}}</li>
			</ul>
			<el-form status-icon label-position="top" :model="ruleForm" :rules="rules" ref="LoginruleForm" label-width="70px"
			 class="demo-ruleForm">
				...
				<el-form-item label="密码" prop="paw">
					<el-input minlength="6" maxlength="20" type="password" placeholder="请填写数字加上字母的6位及以上密码" v-model="ruleForm.paw"></el-input>
				</el-form-item>
				<el-form-item label="重复密码" prop="paws" v-show="modal===`register`">
					<el-input type="password" minlength="6" maxlength="20" placeholder="请输入重复密码" v-model="ruleForm.paws"></el-input>
				</el-form-item>
				<el-form-item label-width="30" label="验证码" prop="ruleCode">
					<el-row :gutter="15">
						<el-col :span="15">
							<el-input minlength="6" type="text" maxlength="6" placeholder="请输入6位数的验证码" v-model="ruleForm.ruleCode"></el-input>
						</el-col>
						<el-col :span="7">
							<el-button @click="getCode(CodeStatus)" :disabled="CodeStatus[`CodeStatus`]" type="success">{{CodeStatus[`title`]}}</el-button>
						</el-col>
					</el-row>
				</el-form-item>
				<el-form-item>
					<el-button type="danger" plain :disabled="ButtonStatus" class="submitForm" @click="submitForm('LoginruleForm')">{{modal === 'login'?"登录":"注册"}}</el-button>
				</el-form-item>
			</el-form>
		</div>
	</div>
